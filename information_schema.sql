...
--
-- Host: localhost:3306
-- Generation Time: Jan 22, 2021 at 05:55 PM
...
CREATE ALGORITHM=UNDEFINED DEFINER=`mysql.infoschema`@`localhost` SQL SECURITY DEFINER VIEW `TABLESPACES_EXTENSIONS`  AS  select `tsps`.`name` AS `TABLESPACE_NAME`,`tsps`.`engine_attribute` AS `ENGINE_ATTRIBUTE` from `mysql`.`tablespaces` `tsps` ;
